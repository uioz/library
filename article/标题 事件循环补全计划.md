...
而微任务是一个简单的概念, 我们从微任务的设计目的上来解释微任务的工作原理.
...
Node.js 官网中[有篇](https://nodejs.org/en/docs/guides/event-loop-timers-and-nexttick/)专门介绍事件循环的文章, 也是这节的**核心**, 这篇文章已经被翻译完成 👉[访问连接](https://segmentfault.com/a/1190000020353471). 本来打算放到这这篇文章中来的但是这样做导致本文太长了, 所以就移除出去了, 是十分重要的内容.

## Node 中的宏任务与微任务

在 Node.js 11 之前的版本中, Node.js 的事件循环是和浏览器有区别的, 在 Node.js 11 后修改了事件循环的方式, 执行效果开始和浏览器一致.

在上一节的 Event Loop 中描述的就是 Node.js 11 中事件循环的执行流程, 如果你仔细阅读其中 `process.nextTick` API 的执行方式就是 "微任务" 的方式.
...
一言以蔽之 Node11 后 `process.nextTick` 的效果就和浏览器中的微任务类似了, Node 在向浏览器规范靠拢.

例如这段代码在 Node11 前执行的输出是:
...
而 `process.nextTick` 就是充当了在Node端中浏览器微任务的角色, 我们可以把这段代码等同于:
